...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty($username_err) && empty($name_err) && empty($school_err) && empty($email_err) && empty($password_err) && empty($confirm_password_err)){
        ...
        $sql = "INSERT INTO users (name, school, username, password, email, hintca) VALUES (?, ?, ?, ?, ?, ?)";

        if($stmt = mysqli_prepare($link, $sql)){
            //Bind variables to the prepared insert statementt as parameters
            mysqli_stmt_bind_param($stmt, "sssssi", $param_name, $param_school, $param_username, $param_password, $param_email, $param_hintca);
